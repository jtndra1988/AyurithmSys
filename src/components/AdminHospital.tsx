...
type ViewMode = 'list' | 'create' | 'analytics' | 'staff' | 'supply-chain';
...
export const AdminHospital: React.FC = () => {
  ...
  const handleCreateHospital = () => {
    if (newHospital.name && newHospital.district) {
      const hospital = {
        ...newHospital,
        id: `H-AP-${Math.floor(Math.random() * 1000)}`,
        activePatients: 0
      } as Hospital;
      ...
      // Reset form
      setNewHospital({ 
        name: '', type: 'District Hospital', district: '', state: 'Andhra Pradesh', address: '',
        bedConfig: { general: 0, icu: 0, hdu: 0, emergency: 0, maternity: 0 },
    ...
    if (selectedHospital && newStaffData.name && newStaffData.department) {
      if (editingStaffId) {
        ...
        const newStaffMember: StaffMember = {
          ...
          id: `S-AP-${Math.floor(Math.random() * 10000)}`,
  ...
  const renderList = () => (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center justify-between">
        <div>
           <h2 className="text-xl font-bold text-slate-800">State Network Facilities</h2>
           <p className="text-sm text-slate-500">Manage Government Hospitals (GGH, District Hospitals, PHCs) across Andhra Pradesh.</p>
        </div>
        <div className="flex gap-2">
           <button 
             onClick={() => setViewMode('supply-chain')}
             className="bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg text-sm font-bold hover:bg-slate-50 flex items-center gap-2 transition-colors"
           >
             <Truck className="h-4 w-4" /> Central Supply Chain
           </button>
           <button 
    ...
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
       <div className="flex items-center justify-between">
          ...
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
             <Truck className="h-6 w-6 text-blue-600" /> Central Supply Chain Intelligence
          </h2>
       </div>

       <div className="bg-blue-50 border border-blue-100 rounded-xl p-6">
          <h3 className="font-bold text-blue-900 mb-2">State Inventory Rebalancing</h3>
          <p className="text-sm text-blue-800 mb-4">
             AI-detected imbalances in critical supplies. Execute transfer orders to move stock from Surplus districts to Deficit districts.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
             <div className="bg-white p-4 rounded-lg border border-blue-200 flex items-center justify-between">
                <div>
                   <p className="text-xs font-bold text-slate-500 uppercase mb-1">Surplus</p>
                   <p className="font-bold text-slate-800">Dengue Kits</p>
                   <p className="text-xs text-emerald-600">Visakhapatnam (+1200)</p>
                </div>
                <ArrowLeft className="h-4 w-4 text-slate-300 rotate-180" />
                <div className="text-right">
                   <p className="text-xs font-bold text-slate-500 uppercase mb-1">Deficit</p>
                   <p className="font-bold text-slate-800">Araku Valley</p>
                   <p className="text-xs text-red-600">Critical Low (&lt;50)</p>
                </div>
             </div>
             ...
             <div className="bg-white p-4 rounded-lg border border-blue-200 flex items-center justify-between">
                <div>
                   <p className="text-xs font-bold text-slate-500 uppercase mb-1">Surplus</p>
                   <p className="font-bold text-slate-800">Insulin (Gla)</p>
                   <p className="text-xs text-emerald-600">Guntur (+500)</p>
                </div>
                <ArrowLeft className="h-4 w-4 text-slate-300 rotate-180" />
                <div className="text-right">
                   <p className="text-xs font-bold text-slate-500 uppercase mb-1">Deficit</p>
                   <p className="font-bold text-slate-800">NTR District</p>
                   <p className="text-xs text-red-600">Low (&lt;20)</p>
                </div>
             </div>

             <div className="flex items-center justify-center">
                <button className="bg-blue-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-blue-700 shadow-sm w-full">
       ...
       <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-200 bg-slate-50">
             <h3 className="font-bold text-slate-700">Regional Stock Levels (Key Items)</h3>
          </div>
          <table className="w-full text-left">
             <thead>
                <tr className="bg-white border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                   ...
                   <th className="px-6 py-4">Total State Stock</th>
                   <th className="px-6 py-4">Visakhapatnam</th>
                   <th className="px-6 py-4">Vijayawada</th>
                   <th className="px-6 py-4">Araku (Tribal)</th>
                   <th className="px-6 py-4 text-right">Action</th>
                </tr>
             </thead>
             <tbody className="divide-y divide-slate-100">
                <tr className="hover:bg-slate-50">
                   <td className="px-6 py-4 font-medium text-slate-900">Dengue NS1 Kit</td>
                   <td className="px-6 py-4 font-bold">15,240</td>
                   <td className="px-6 py-4 text-emerald-600 font-bold">3,500</td>
                   <td className="px-6 py-4 text-slate-600">1,200</td>
                   <td className="px-6 py-4 text-red-600 font-bold">45</td>
                   <td className="px-6 py-4 text-right">
                      <button className="text-xs font-bold text-blue-600 hover:underline">Rebalance</button>
                   </td>
                </tr>
                <tr className="hover:bg-slate-50">
                   <td className="px-6 py-4 font-medium text-slate-900">Rabies Vaccine</td>
                   <td className="px-6 py-4 font-bold">8,400</td>
                   <td className="px-6 py-4 text-slate-600">2,100</td>
                   <td className="px-6 py-4 text-emerald-600 font-bold">3,200</td>
                   <td className="px-6 py-4 text-amber-600 font-bold">120</td>
                   <td className="px-6 py-4 text-right">
                      <button className="text-xs font-bold text-blue-600 hover:underline">Rebalance</button>
                   </td>
                </tr>
    ...
    return (
      <div className="max-w-4xl mx-auto animate-in slide-in-from-right-4 duration-300">
        <div className="mb-6 flex items-center justify-between">
           ...
           <h2 className="text-xl font-bold text-slate-800">Register New AP Facility</h2>
        </div>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                         ...
                         <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-slate-50" value="Andhra Pradesh" readOnly />
                      </div>
                      <div className="space-y-2">
                         <label className="text-sm font-semibold text-slate-700">District <span className="text-red-500">*</span></label>
                         <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none" placeholder="e.g. Visakhapatnam, Guntur" value={newHospital.district} onChange={e => setNewHospital({...newHospital, district: e.target.value})} />
                      </div>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                         ...
                         <input type="email" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none" placeholder="admin@hospital.ap.gov.in" value={newHospital.contactEmail} onChange={e => setNewHospital({...newHospital, contactEmail: e.target.value})} />
                      </div>
                      <div className="space-y-2">
                   ...
                   <div className="pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                         ...
                         <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none font-mono" placeholder="IN-AP-XXXX" value={newHospital.abdmId} onChange={e => setNewHospital({...newHospital, abdmId: e.target.value})} />
                      </div>
                   </div>
                </div>
             )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="email" className="w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none" placeholder="official@ap.gov.in" value={newStaffData.email} onChange={e => setNewStaffData({...newStaffData, email: e.target.value})} />
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none font-mono placeholder:font-sans" placeholder="e.g. APMC-2340-22" value={newStaffData.registrationNumber} onChange={e => setNewStaffData({...newStaffData, registrationNumber: e.target.value})} />
                    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {viewMode === 'supply-chain' && renderSupplyChain()}
    </div>
  );
};
